...
function isRateLimitMsg(msg: string): boolean {
  ...
  return l.includes("rate limit") || l.includes("too many") ||
    l.includes("security purposes") || l.includes("exceeded");
}
...
function Field({
  ...
  return (
    <div style={{ position: "relative" }}>
      <div style={{ position: "absolute", left: 16, top: "50%", transform: "translateY(-50%)", color: ASH, pointerEvents: "none", display: "flex" }}>
        {icon}
      </div>
      <input
        type={type}
        ...
        onFocus={(e) => { e.target.style.border = `1.5px solid ${FIRE}`; e.target.style.background = WHITE; }}
        onBlur={(e) => { e.target.style.border = `1px solid ${EMBER}`; e.target.style.background = SAND; }}
      />
      {right && (
...
export default function AuthPage() {
  ...
  const applyError = (msg: string) => {
    if (isRateLimitMsg(msg)) {
      const secs = parseRateLimitSeconds(msg) ?? 60;
      setRateLimitSecs(secs);
      setError("__rate_limited__");
    } else {
      setError(msg);
    }
  };
  ...
  const switchMode = (m: Mode) => { setMode(m); setError(""); setRateLimitSecs(0); };
  ...
  return (
    <div style={{ minHeight: "100vh", background: COAL, display: "flex" }}>
      ...
      <div
        className="auth-left"
        style={{
          display: "none", width: 340, flexShrink: 0,
          background: "#110D06",
      ...
      >
        <div>
          <div style={{ width: 48, height: 48, borderRadius: 11, background: FIRE, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <span style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: 20, color: "white", letterSpacing: "-0.03em" }}>S1</span>
          </div>
          <div style={{ marginTop: 14, fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: 20, color: "white", letterSpacing: "-0.03em" }}>Sth1r</div>
          <div style={{ fontSize: 10, letterSpacing: "0.12em", color: "rgba(255,255,255,0.3)", fontFamily: "'DM Sans', sans-serif", marginTop: 3 }}>by Sthappit</div>
        </div>
        <div>
          <p style={{ fontFamily: "'Instrument Serif', serif", fontStyle: "italic", fontSize: 24, color: "white", lineHeight: 1.5, fontWeight: 400 }}>
            Billing that works<br />even offline.
          </p>
          <p style={{ fontSize: 12, color: "rgba(255,255,255,0.3)", marginTop: 14, lineHeight: 1.7, fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 20px", overflowY: "auto" }}>
        ...
        {/* Mobile logo */}
        <div className="auth-mobile-logo" style={{ marginBottom: 32, display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
          <div style={{ width: 56, height: 56, borderRadius: 14, background: FIRE, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <span style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: 22, color: "white", letterSpacing: "-0.03em" }}>S1</span>
          </div>
          <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: 18, color: "white", letterSpacing: "-0.02em" }}>Sth1r</div>
          <div style={{ fontSize: 10, letterSpacing: "0.12em", color: "rgba(255,255,255,0.3)", fontFamily: "'DM Sans', sans-serif" }}>by Sthappit</div>
        </div>

        {/* Card */}
        <div style={{ width: "100%", maxWidth: 420, background: WHITE, borderRadius: 20, border: `1px solid ${EMBER}`, overflow: "hidden", boxShadow: "0 8px 32px rgba(26,18,8,0.10)" }}>
          ...
          <div style={{ padding: "28px 28px 32px" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 16 }}>
              ...
              <Field icon={<User size={15} />} placeholder="Username" value={username} onChange={setUsername} />
              ...
              <Field
                ...
                right={
                  <button onClick={() => setShowPwd((p) => !p)} style={{ background: "none", border: "none", cursor: "pointer", color: ASH, display: "flex", padding: 0 }}>
              ...
              {mode === "signup" && (
                <>
                  <Field icon={<Store size={15} />} placeholder="Business Name *" value={businessName} onChange={setBusinessName} />
                  <Field icon={<User size={15} />} placeholder="Owner Name" value={ownerName} onChange={setOwnerName} />

                  {/* Business type */}
                  <div style={{ position: "relative" }}>
                    <Store size={15} style={{ position: "absolute", left: 16, top: "50%", transform: "translateY(-50%)", color: ASH, pointerEvents: "none" }} />
                    <select
                      value={bizType}
                      onChange={(e) => setBizType(e.target.value)}
                      style={{
                        width: "100%", height: 48, paddingLeft: 44, paddingRight: 36,
                        borderRadius: 50, border: `1px solid ${EMBER}`, background: SAND,
                        fontSize: 14, color: COAL, fontFamily: "'DM Sans', sans-serif",
                        fontWeight: 500, outline: "none", appearance: "none", cursor: "pointer",
                      }}
                    >
            ...
            {error && (
              <div
                style={{
                  fontSize: 12, color: FIRE6, background: FIRE50, borderRadius: 12,
              ...
              >
                <span>{isRateLimited ? "Too many attempts — please wait:" : error}</span>
                {isRateLimited && (
                  <span style={{
            ...
            >
              ...
              {isRateLimited ? `Wait ${rateLimitSecs}s…` : mode === "signin" ? "Sign In →" : "Create Account →"}
            </button>

            <p style={{ textAlign: "center", fontSize: 12, color: ASH, marginTop: 20, fontFamily: "'DM Sans', sans-serif" }}>
              {mode === "signin" ? "New to Sth1r? " : "Already have an account? "}
              <button
                onClick={() => switchMode(mode === "signin" ? "signup" : "signin")}
                style={{ background: "none", border: "none", cursor: "pointer", color: FIRE6, fontSize: 12, fontFamily: "'DM Sans', sans-serif", fontWeight: 600 }}
              >
        ...
        <p style={{ fontSize: 11, color: "rgba(255,255,255,0.18)", marginTop: 32, letterSpacing: "0.08em", fontFamily: "'DM Sans', sans-serif" }}>
          Sth1r · by Sthappit
        </p>
      </div>
