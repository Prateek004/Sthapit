...
import { fmtRupee, fmtDate, toP, calcExactDenominations } from "@/lib/utils";
  ...
  const handleWhatsApp = () => {
    ...
    const lines = [
      ...
      ...order.items.map((i) => {
        ...
        const portionStr = i.selectedPortion
          ? ` (${i.selectedPortion})`
          : i.selectedSize
          ? ` (${i.selectedSize})`
          : "";
        const addOnsStr =
          i.selectedAddOns.length > 0
            ? ` [Add-ons: ${i.selectedAddOns
                .map((a) => `${a.name} +${fmtRupee(a.pricePaise)}`)
                .join(", ")}]`
            : "";
        const notesStr = i.notes ? ` (Note: ${i.notes})` : "";
        return `• ${i.name}${portionStr} x${i.qty}${addOnsStr}${notesStr}  ${fmtRupee(
          ...
          {(method === "credit_card" || method === "debit_card" || method === "card") && (
            <div className="bg-purple-50/70 border border-purple-200/70 rounded-2xl p-4 text-center space-y-1.5">
              ...
              <p className="text-xs text-gray-500 font-medium">
                Swipe or tap card on POS terminal and tap Collect &amp; Close below
              </p>
            </div>
          )}
